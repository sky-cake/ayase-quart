...
from asyncio import run as run_a
...
async def get_post_rows(board: str, thread_nums: list[int] | None=None, get_row_keys: bool=False):
    """This query is used for fetching post rows, or the columns returned in post rows.
    This is done in the same function based off the `get_row_keys` flag so as to minimize code maintenance.
    """
    ...
    if not get_row_keys:
        ...
        q_suffix = f"""where thread_num in ({placeholders})"""
    ...
    await db.connect()
    results = await db.query_execute(q, fetchone=True)
    column_names = tuple(col for col in results)
    await db.disconnect()
    return column_names


async def get_post_row_keys():
    return await get_post_rows(CONSTS.boards_in_database[0], None, True)


row_keys = run_a(get_post_row_keys())
