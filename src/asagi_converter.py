...
async def get_post_images(board: str, post_num: int):
    ...
    sql = SELECT_POST_IMAGES.format(
        board=board,
        post_num=post_num,
        image_selector=SHA256_IMAGE_SELECTOR if configs.hash_format == 'sha256' else MD5_IMAGE_SELECTOR
    )
    return await query_handler(sql, fetchall=False)
...
async def get_op_details(board: str, thread_nums: List[int]):
    if not thread_nums:
...
async def get_gallery_images(board: str, page_num: int):
    ...
    selector = SELECT_GALLERY_THREAD_IMAGES_SHA256 if configs.hash_format == 'sha256' else SELECT_GALLERY_THREAD_IMAGES_MD5
...
async def generate_gallery(board_name: str, page_num: int):
    ...
    thread_list, details, images = await asyncio.gather(
        get_gallery_threads(board_name, page_num),
        get_gallery_details(board_name, page_num),
        get_gallery_images(board_name, page_num)
    )
...
async def convert_post(board_name: str, post_id: int):
    post, images = await asyncio.gather(
        get_post(board_name, post_id),
        get_post_images(board_name, post_id)
    )
    post = [post]
    images = [images]
...
async def convert_thread_ops(board_name: str, page_num: int):
    ...
    if not op_list:
        return convert([], [], [], isOPs=True)

    op_image_list, op_detail_list = await asyncio.gather(
        get_op_images(board_name, [op.md5 for op in op_list]),
        get_op_details(board_name, [op.no for op in op_list])
    )
...
async def convert_thread_preview(board_name: str, thread_id: int):
    thread, images = await asyncio.gather(
        get_thread_preview(board_name, thread_id),
        get_thread_preview_images(board_name, thread_id)
    )
...
async def convert_thread(board_name: str, thread_id: int):
    thread, images, details = await asyncio.gather(
        get_thread(board_name, thread_id),
        get_thread_images(board_name, thread_id),
        get_thread_details(board_name, thread_id), # details needs to be an array
    )
    details = [details]
